...
* Support token expiration and optional refresh mechanism.
...
### 2.4 User Management
Accessible only to Administrators. Other users do not have the ability to update others or their profile.
...
### 2.5 User Onboarding & Password Storage
**Upon user creation:**
* System shall send an email with username and temporary password.

**On first login:**
* Users must reset password (mandatory) before accessing the system.
* Password policies must be enforced (Minimum length and complexity enforced).

**Storing Passwords:**
* Passwords shall be stored using secure hashing (e.g., bcrypt).
* Plain-text passwords shall never be stored.
* All operations shall include: Validation, Error handling, and Success confirmations.
...
## 4. Real-Time Features (WebSocket-Based)
The system shall use WebSockets to push updates without polling.

### 4.1 Events and Notifications
...
The system shall enforce validation at both Frontend (immediate feedback) and Backend (final validation layer).
...
### 5.1 Error Handling (Backend)
The API shall return Standard HTTP status codes with Structured error response as required.
Structured error response should have:
* Error code
* Message
...
* **API Documentation:** Use Swagger/OpenAPI for documenting REST endpoints.
* **README.md:** Must include Project overview, Technologies used, Setup instructions, API usage (link to Swagger UI), and Team member contributions.
